...
void set_oldfriends(vtable &names, adjtable &friends, int M0, int M1) {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int j = 0; j < num; ++j) {
      ...
      friends[i].push_back(do_know[j]);
      friends[do_know[j]].push_back(i);
    }
  }
  for (int i = 0; i < N; ++i) {
    sort(friends[i].begin(), friends[i].end());
    friends[i].erase(unique(friends[i].begin(), friends[i].end()),
                     friends[i].end());
  }
}
  ...
  for (int i = 0; i < N; ++i) {
    for (int j = 0; j < N; ++j) {
      if (count(friends[i].begin(), friends[i].end(), j)) {
        for (int k = 0; k < N; ++k) {
          if (i != k && count(friends[j].begin(), friends[j].end(), k) &&
              !count(friends[i].begin(), friends[i].end(), k))
