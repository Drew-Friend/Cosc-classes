...
public:
  ...
  bool operator<(const namescores_t &other) const {
    if (!(scores < other.scores) && !(other.scores < scores))
...
int main(int argc, char **argv) {
  ...
  if (argc != 3) {
    // I wasn't able to run the solution executables on hydra, so I guessed what
  ...
  while (!fin.eof()) {
    ...
    if (!fin.eof()) {
      for (vector<namescores_t>::iterator ptr = NS.begin(); ptr != NS.end();
           ++ptr)
        if (!(*ptr < sNS) && !(sNS < *ptr))
          seen = true;
      if (!seen)
