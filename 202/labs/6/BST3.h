...
void bst<TKey>::node::print()
{
  cout << ID;
  cout << setw(3) << key << " :";
  if(pLink) cout << "P=" << setw(3) << pLink->ID;
  else        cout << "ROOT ";
...
class bst<TKey>::iterator *bst<TKey>::iterator::operator++(){
  ...
  // If there's a right branch, take it, then go to the leftmost point from there
  if(!(p->link[1]))
  { 
    if(!p->pLink)
    {
...
class bst<TKey>::iterator bst<TKey>::upper_bound(const TKey & val){
  ...
  while(!done) {
    ...
    if(view.p->key > val && (view.p->key < standard->key) == (standard->key > val))
      standard = view.p;
  }
