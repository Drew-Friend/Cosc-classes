...
int sudoku::square_index(int i, int j)
{
  return 3 * (i / 3) + (j / 3);
}
...
{
  // I used all class variables, so I didn't need arguements, but because there are 2 functions named solve it had to have an arguement to distinguish it
  ...
  // For each possible value of the cell with the least options, attempt to solve the rest of the puzzle.
  ...
  // If no valid value in this square completes the game, empty it and go up a level
  return false;
}
...
void sudoku::read(const char *fname)
{
  ...
  while (fin >> i >> j >> value)
  {
    ...
    {
      cout << "line " << line << ": " << i << ' ' << j << ' ' << value << " out-of-bounds grid index\n";
      err = true;
      continue;
    }
    set_inuse(i, j, value);
...
{
  // For each possible number, if it is not found in the row, column, or square, add it to the list
  for (int c = 1; c < 10; ++c)
  {
    if (inuse[i][c] & ROW_INUSE ||
        inuse[j][c] & COLUMN_INUSE ||
...
void sudoku::error_check(int mode)
{
  ...
  for (int i = 0; i < 81; ++i)
  {
    ...
    for (int j = 0; j < 9; ++j)
    {
      ...
      if (game[(j / 3) + 3 * (row / 3)][(j % 3) + 3 * (column / 3)] == val && j != spos)
      {
        // cout << pH[0] << pH[1] << pH[2] << '\n';
