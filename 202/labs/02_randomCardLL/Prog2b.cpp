...
void list::insert(const int &din) {
  ...
  // Cut stops if it sees the value currently being added to the list, for
  // reference Ref does not stop, and is used to find the end of the string
  for (int i = 0; i < N; i++) {
  ...
  // Make a temporary reference so the node can be cut out of place, and then
  // referenced again, if a value was found Corner case when the new element is
  // already the last in the list. Nothing needs to happen
